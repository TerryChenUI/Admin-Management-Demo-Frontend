...
class CategoryEdit extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.created.data || nextProps.created.error) {
            nextProps.created.data ? alertService.success('添加成功', null, '/category/list') : alertService.error('添加类别失败', nextProps.created.error);
        } else if (nextProps.updated.data || nextProps.updated.error) {
            nextProps.updated.data ? alertService.success('更新成功', null, '/category/list') : alertService.error('更新类别失败', nextProps.updated.error);
        }
    }
